          ...
          .map((pedido) => {
            ...
            return (
              ...
              <div className="flex-shrink-0 flex flex-col items-end space-y-2 sm:min-w-[120px]">
                  ...
                  {SIGUIENTE[pedido.estado_codigo] && (
                    <button
                      onClick={() => avanzarEstado(pedido.id, pedido.estado_codigo)}
                      className="w-full border border-stone-900 hover:bg-sky-400 hover:border-sky-400 text-stone-700 font-bold text-[11px] py-1.5 px-3 rounded-lg transition-all duration-300 active:scale-95 focus:outline-none cursor-pointer text-center whitespace-nowrap"
                    >
                      {pedido.estado_codigo === 'PENDIENTE' ? 'Aceptar Pedido' : 'Avanzar Estado'}
                    </button>
                  )}
