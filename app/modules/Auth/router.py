...
def _autenticar(session: Session, email: str, password: str) -> tuple[str, list]:
    ...
    token = create_access_token(
        {"sub": user.email, "roles": list(roles)},
...
def token(form: OAuth2PasswordRequestForm = Depends(), session: Session = Depends(get_session)):
    # OAuth2 usa username field, pero aquí esperamos el email
...
def login(data: LoginRequest, response: Response, session: Session = Depends(get_session)):
    jwt, roles = _autenticar(session, data.email, data.password)
    response.set_cookie(
        key="access_token",
        value=jwt,
        httponly=True,
    ...
    return {"access_token": jwt, "token_type": "bearer", "roles": roles}
